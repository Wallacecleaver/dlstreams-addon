...
# ============================================================================
# TOUS LES IDs DES CHAINES FRANCAISES (extraits de dlstreams.st/24-7-channels.php)
# ============================================================================
FRENCH_CHANNEL_IDS = {
    # TNT / Generales
    "469",   # TF1 France
...
def channels() -> list[dict]:
    """Annuaire dlstreams -> [{id, name}]. Filtre chaines francaises uniquement."""
    now = time.time()
...
def resolve_player(stream_url: str) -> tuple[str, str]:
    ...
    if not mu:
        ...
        if not mu:
            raise ValueError("m3u8 introuvable")
    return mu, host
