#!/usr/bin/env python3
"""W Addon TV -> Stremio : mini-addon + proxy autonome (stdlib pure, ZERO dependance).
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _manifest(self) -> dict:
        ...
        return {
            "id": "st.waddontv.proxy",
            "version": "1.1.0",
            "name": "W Addon TV",
            "description": "Chaînes live (DaddyLive via dlstreams) + Vavoo, proxifiées (headers + content-type "
                           "corrigés). Sans MediaFlow.",
            "resources": ["catalog", "meta", "stream"],
            "types": ["tv"],
            "idPrefixes": ["dlstreams:", "vavoo:"],
            "catalogs": [{"type": "tv", "id": "dlstreams", "name": "W Addon TV",
...
def main():
    print(f"W Addon TV addon+proxy sur http://0.0.0.0:{PORT}")
