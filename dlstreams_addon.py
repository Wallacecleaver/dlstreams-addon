...
<!-- Modal chaîne personnalisée -->
<div class="modal" id="tv-custom-modal">
<div class="modal-content tv-modal">
<div class="modal-header">
<h3 id="tv-custom-modal-title">Ajouter chaîne personnalisée</h3>
<button class="modal-close" onclick="closeCustomChannelModal()">&times;</button>
</div>
<div class="modal-body">
<input type="hidden" id="tv-custom-id">
<div class="form-row">
<label>ID de la chaîne</label>
<input type="text" id="tv-custom-id-input" placeholder="mon-canal-perso">
<span style="font-size:11px;color:var(--muted)">Lettres, chiffres, _ et - uniquement</span>
</div>
<div class="form-row">
<label>Nom de la chaîne</label>
<input type="text" id="tv-custom-name" placeholder="Ma Chaîne">
</div>
<div class="form-row">
<label>Logo (URL png/jpg)</label>
<input type="url" id="tv-custom-logo" placeholder="https://.../logo.png">
<img id="tv-custom-logo-preview" src="" alt="" style="width:48px;height:27px;object-fit:cover;border-radius:4px;border:1px solid var(--border);margin-top:6px;display:none">
</div>
<div class="form-row">
<label>Flux (un par ligne)</label>
<input type="text" id="tv-custom-streams" placeholder="https://.../stream1.m3u8\nhttps://.../stream2.m3u8" style="min-height:80px;font-family:var(--font-mono)">
<span style="font-size:11px;color:var(--muted)">Un flux par ligne</span>
</div>
<div class="form-row">
<label>EPG (ID xmltv)</label>
<input type="text" id="tv-custom-epg" placeholder="TF1.fr">
</div>
<div class="form-actions">
<button class="btn-outline-sm" onclick="closeCustomChannelModal()">Annuler</button>
<button class="btn-outline-sm" onclick="deleteCustomChannel()" id="tv-custom-del-btn" style="display:none">🗑️ Supprimer</button>
<button class="add-source-btn" onclick="saveCustomChannel()">💾 Enregistrer</button>
</div>
</div>
</div>
</div>
...
let _lastPage = null;
let _inNavigate = false;
function navigateTo(page) {
if (_inNavigate) return;
_inNavigate = true;
_lastPage = page;
...
if (p && p !== _lastPage && document.getElementById('page-' + p) && !_inNavigate) navigateTo(p);
});
...
(c.name||'').toLowerCase().includes(q) ||
(c.src||'').toLowerCase().includes(q) ||
(c.auto_epg||'').toLowerCase().includes(q)
);
if(cnt) cnt.textContent = channels.length + (channels.length>1?' chaînes':' chaîne');
if(!channels.length){
...
const epgHtml = c.override_epg ? `<span class="tv-epg ok">${escapeHtml(c.override_epg)}</span>` : (c.auto_epg ? `<span class="tv-epg warn">${escapeHtml(c.auto_epg)}</span>` : `<span class="tv-epg missing">—</span>`);
...
.copy{display:inline-flex;align-items:center;gap:6px;margin-top:12px;padding:10px 16px;border:1px solid var(--border);border-radius:8px;background:rgba(255,255,255,.02);color:var(--muted);font-size:13
